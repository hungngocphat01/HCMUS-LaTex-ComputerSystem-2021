...
\draw [arrow] (if.south) |- ([yshift=-2cm]stop.south) -- (stop.south);
...
\draw [arrow] (else.east) -- ([yshift=-2.5cm]stop.south) -- (stop.south);
\draw [arrow] (then) -- (stop);
\end{tikzpicture}
\end{figure}
...
\draw [arrow] (thenn.east) -- ([yshift=2cm]stop.north) -- (stop.north);
\draw [arrow] (then1.east) -- +(3.03cm, 0) -- (stop.north);
\draw [arrow] (then2.east) -- +(3.02cm, 0) -- (stop.north);
...
\draw [arrow] (cond) |- node[anchor=west] {Thoả} +(-8, -2.5) |- (loop.west);
